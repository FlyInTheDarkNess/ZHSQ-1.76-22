...
@interface PropertyNoticeViewController ()
{
...
@synthesize leftSwipeGestureRecognizer,tableview;
...
//    bounds.size.height -= 44.f;
    ...
    [shequBtn addTarget:self action:@selector(shequtongzhi) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:shequBtn];
    
    labelline=[[UILabel alloc]initWithFrame:CGRectMake(45, 90, 70, 2)];
...
-(void)shequtongzhi
{
    CommunityNoticeViewController *tongzhi=[[CommunityNoticeViewController alloc]init];
...
-(void)addData
{
    ...
    NSString *str1=[NSString stringWithFormat:@"{\"city_id\":\"101\",\"community_id\":\"102\",\"xiaoqu_id\":\"all\",\"id_array\":%@}",[self getFromArr:idList withNumber: startIndex + 5]];
    NSString *str_jiami=[SurveyRunTimeData hexStringFromString:str1];
    sqHttpSer = [[BulletinNoticeHttpService alloc]init];
    sqHttpSer.strUrl = WuYeTongZhi_m12_02;
    sqHttpSer.delegate = self;
    sqHttpSer.requestDict = [NSDictionary dictionaryWithObjectsAndKeys:str_jiami,@"para", nil];
    [sqHttpSer beginQuery];
    isUp = NO;
...
{
    TZDictionary=[[NSDictionary alloc]init];
    TZDictionary=newsArr[indexPath.row];
    TongZhiXiangQingViewController *xiangqing=[[TongZhiXiangQingViewController alloc]init];
...
-(void)didReceieveSuccess:(NSInteger)tag{
    switch (tag) {
        case 1:{
            ...
            NSString *str1=[NSString stringWithFormat:@"{\"city_id\":\"101\",\"community_id\":\"102\",\"xiaoqu_id\":\"all\",\"id_array\":%@}",[self getFromArr:idList withNumber: startIndex + 10]];
            NSString *str_jiami=[SurveyRunTimeData hexStringFromString:str1];
            sqHttpSer = [[BulletinNoticeHttpService alloc]init];
            sqHttpSer.strUrl = WuYeTongZhi_m12_02;
            sqHttpSer.delegate = self;
