...
-(void)MyReplyDataInit
{
    ...
    sqHttpSer.strUrl = MyPosts_m10_08;
...
-(void)MyReplyAddData
{
    ...
    NSString *str_jiami=[SurveyRunTimeData hexStringFromString:str1];
    sqHttpSer = [[PersonCenterHttpService alloc]init];
    sqHttpSer.strUrl = MyPosts_m10_11;
    sqHttpSer.delegate = self;
    sqHttpSer.requestDict = [NSDictionary dictionaryWithObjectsAndKeys:str_jiami,@"para", nil];
    [sqHttpSer beginQuery];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *text = [CustomMethod escapedString:[arr_MyPosts[indexPath.row] objectForKey:@"title"]];
    ...
    NSString *text1 = [CustomMethod escapedString:[arr_MyPosts[indexPath.row] objectForKey:@"content"]];
    ...
    label_time.text=[arr_MyPosts[indexPath.row] objectForKey:@"create_time"];
    ...
    pinglunshu.backgroundColor=[UIColor whiteColor];
    pinglunshu.text=[NSString stringWithFormat:@"评论 %@",[arr_MyPosts[indexPath.row] objectForKey:@"feedback_num"]];
    ...
    NSArray *array=[arr_MyPosts[indexPath.row] objectForKey:@"images"];
    if (array.count>0)
    {
...
-(void)shanchu:(AXHButton *)btn
{
    ...
    tieziID=[arr_MyPosts[btn.tag] objectForKey:@"id"];
    ...
    //NSLog(@"canshu :%@",str1);
    sqHttpSer = [[PersonCenterHttpService alloc]init];
    sqHttpSer.strUrl = SheQuHuDong_m10_09;
    sqHttpSer.delegate = self;
    sqHttpSer.requestDict = [NSDictionary dictionaryWithObjectsAndKeys:str_jiami,@"para", nil];
    [sqHttpSer beginQuery];
}
...
-(void)didReceieveSuccess:(NSInteger)tag{
    switch (tag) {
        case 9:{
            ...
            NSLog(@"我的帖子 :%@",sqHttpSer.responDict);
             intb = [a intValue];
            if (intb==3007)
            {
        ...
        case 10:{
            ...
            NSLog(@"我的帖子2:%@",sqHttpSer.responDict);
            NSString *a=sqHttpSer.responDict[@"ecode"];
             intb = [a intValue];
            if (intb==3007)
            {
        ...
        case 26:
        {
            NSLog(@"删除帖子:%@",sqHttpSer.responDict);
            NSString *a=sqHttpSer.responDict[@"ecode"];
            intb = [a intValue];
            if (intb==4000)
            {
