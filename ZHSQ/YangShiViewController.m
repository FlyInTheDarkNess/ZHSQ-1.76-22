...
@interface YangShiViewController ()

@end

@implementation YangShiViewController
...
- (void)viewDidLoad
{
    ...
    yangshi_jimudan=[[NSArray alloc]init];
...
-(NSInteger)weekDate:(NSDate*)date
{
    ...
    [HttpPostExecutor postExecuteWithUrlStr:ShouShiZhiNan_m8_01 Paramters:str_jiami FinishCallbackBlock:^(NSString *result){
       ...
       // NSLog(@"第2次: %@", result);//result就是NSString类型的返回值
        ...
        if (intb==1000)
        {
            yangshi_jimudan=[rootDic objectForKey:@"content_info"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    shijian.text=[yangshi_jimudan[indexPath.row] objectForKey:@"tv_time"];
    ...
    jiemu.text=[yangshi_jimudan[indexPath.row] objectForKey:@"tv_content"];
