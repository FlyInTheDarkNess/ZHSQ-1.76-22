//
//  ShanDongweishiViewController.m
//  ZHSQ
//
...
@interface ShanDongweishiViewController ()

@end

@implementation ShanDongweishiViewController
...
- (void)viewDidLoad
{
    ...
    shandong_jiemudan=[[NSArray alloc]init];
...
-(void)initBase
{
    ...
    _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 55, 320, 80)];
...
-(void)initDateView
{
    ...
    for (int i = 0; i < 7; i++)
    {
        UIButton* lab = [[UIButton alloc]initWithFrame:CGRectMake(320/7*i, -10, 320/7, 30)];
...
-(NSInteger)weekDate:(NSDate*)date
{
    ...
    [HttpPostExecutor postExecuteWithUrlStr:ShouShiZhiNan_m8_01 Paramters:str_jiami FinishCallbackBlock:^(NSString *result){
        ...
        if (intb==1000)
        {
            shandong_jiemudan=[rootDic objectForKey:@"content_info"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    shijian.text=[shandong_jiemudan[indexPath.row] objectForKey:@"tv_time"];
    ...
    jiemu.text=[shandong_jiemudan[indexPath.row] objectForKey:@"tv_content"];
...
{
    return shandong_jiemudan.count;
}
//返回多少个section
