//
//  WoDeTingCheFeiViewController.m
//  ZHSQ
//
...
#import "WoDeTingCheFei_YiJiaoViewController.h"
#import "WoDeTingCheFei_WeiJiaoViewController.h"
...
@interface WoDeTingCheFeiViewController ()

@end

@implementation WoDeTingCheFeiViewController
    ...
    @try
    {
        ...
        [HttpPostExecutor postExecuteWithUrlStr:KuaiJieJiaoFeiZhuJian_m30_01 Paramters:Str FinishCallbackBlock:^(NSString *result){
            ...
            if (intb==1000)
            {
                ...
                NSString *Str;
                Str=[str2 stringByAppendingString:str_jiami];
                [HttpPostExecutor postExecuteWithUrlStr:TingCheFei_m30_05 Paramters:Str FinishCallbackBlock:^(NSString *result)
                 {
                     if (result.length<=0)
                     {
                         UIAlertView * aler = [[UIAlertView alloc] initWithTitle:@"更新数据失败" message:@"网络信号不佳，请选择好的网络" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                    ...
                    NSString *str_tishi=[WYF_WeiJiaoDictionary objectForKey:@"ecode"];
                    int inta =[str_tishi intValue];
                    if (inta==1000)
                    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:tableview_lishizhangdan])
    {
        ...
        WoDeTingCheFei_YiJiaoViewController *xiangqing=[[WoDeTingCheFei_YiJiaoViewController alloc]init];
    ...
    else
    {
        ...
        WoDeTingCheFei_WeiJiaoViewController *xiangqing=[[WoDeTingCheFei_WeiJiaoViewController alloc]init];
        [self presentViewController:xiangqing animated:NO completion:nil];
