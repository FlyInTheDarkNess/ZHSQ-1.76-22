...
#import "FangChanYeWuViewController.h"
...
#import "FangChanYeWuXiangQingViewController.h"
#import "URL.h"
#import "JiaMiJieMi.h"
#import "fuwu.h"
NSString *const FangChanYeWuMJTableViewCellIdentifier = @"Cell";
extern NSDictionary *FangChanYeWuDictionary;
@interface FangChanYeWuViewController ()
{
...
@implementation FangChanYeWuViewController
...
- (void)dataInit
{
    ...
    if([CheckNetwork isExistenceNetwork]==1)
    {
        ...
        NSString *str1=@"{\"session\":\"\",\"city_id\":\"101\",\"area_id\":\"\",\"agency_id\":\"\",\"community_id\":\"\",\"quarter_id\":\"\",\"article_type_id\":\"43\"}";
        ...
        [HttpPostExecutor postExecuteWithUrlStr:ZiXunXinXi_m38_01 Paramters:Str FinishCallbackBlock:^(NSString *result){
            ...
            if ([arr count]==0)
            {
                ...
                __block FangChanYeWuViewController *bSelf=self;
                ...
                [HttpPostExecutor postExecuteWithUrlStr:ZiXunXinXi_m38_02                                              Paramters:Str
                                    FinishCallbackBlock:^(NSString *result)
                 {
                     ...
                     if (intString==1000)
                     {
                         ...
                         // 2.初始化假数据
                         [_fakeData addObjectsFromArray:info];
                     }
                     }
                 }];
...
- (void)viewDidLoad
{
    ...
    label=[[UILabel alloc]initWithFrame:CGRectMake(0, 20, 320, 35)];
    ...
    button=[[UIButton alloc]initWithFrame:CGRectMake(0, 20, 35, 35)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *imageurl=[[[_fakeData[indexPath.row] objectForKey:@"pic"] objectAtIndex:0] objectForKey:@"thumbs_url"];
    ...
    if ((imageurl.length>0&&![imageurl isEqualToString:@""])&&(imageurl!=nil)&&(data.length>0))
    {
        ...
        imageView.frame=CGRectMake(5, 3, 44, 44);
        [imageView setPlaceHolder:[UIImage imageNamed:@"fangchan1"]];
        [imageView setImageWithPath:imageurl];
        [cell addSubview:imageView];
        
        UILabel *name=[[UILabel alloc]initWithFrame:CGRectMake(50, 3, 250, 20)];
        ...
        UILabel *publish_department=[[UILabel alloc]initWithFrame:CGRectMake(50, 30, 180, 20)];
        publish_department.text=[_fakeData[indexPath.row] objectForKey:@"summary"];
        publish_department.font=[UIFont systemFontOfSize:14];
    ...
    UILabel *publish_time=[[UILabel alloc]initWithFrame:CGRectMake(240, 30, 80, 20)];
    publish_time.text=[_fakeData[indexPath.row] objectForKey:@"article_date"];
    publish_time.font=[UIFont systemFontOfSize:14];
    publish_time.textColor=[UIColor grayColor];
    [cell addSubview:publish_time];
    ...
    UILabel *publish_department=[[UILabel alloc]initWithFrame:CGRectMake(15, 30, 125, 20)];
    publish_department.text=[_fakeData[indexPath.row] objectForKey:@"summary"];
    publish_department.font=[UIFont systemFontOfSize:14];
    publish_department.textColor=[UIColor grayColor];
    [cell addSubview:publish_department];
...
{
    FangChanYeWuDictionary=_fakeData[indexPath.row];
    FangChanYeWuXiangQingViewController *xiangqing=[[FangChanYeWuXiangQingViewController alloc]init];
